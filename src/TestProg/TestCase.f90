...
PROGRAM h5_cmprss
  ...
  ! Create dataset "Data" in the group using absolute name.
  dims(1:2) = (/dim0, dim1/)
  ...
  ! Create data set
  CALL h5dcreate_f(file_id, "Data", H5T_NATIVE_INTEGER, dataspace_id, &
  ...
  CALL h5fclose_f(file_id, error)
      ...
      ! free up memory.  More important later.
   CALL h5garbage_collect_f(error)     ! Returns 0 if good, -1 on fails.
   if (error .ne. 0) print*,'Garbage collection failed. Error: ',error
